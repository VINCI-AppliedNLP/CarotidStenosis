...
public class StenosisService {
    ...
    public static void main(String[] args) {
        StenosisService current_service = new StenosisService();

        if (args.length == 0) {
            current_service.serviceConfigFile = new File[]{new File("config/ServiceConfig.groovy")};
            current_service.pipeline = new String[]{"gov.va.vinci.pipeline.Carotid_Stenosis_Pipeline"};
    ...
    public static void printUsage() {
        CmdLineParser parser = new CmdLineParser(new StenosisService());
        System.out.print("Usage: java " + StenosisService.class.getCanonicalName());
